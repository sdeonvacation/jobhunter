...
public class VisaSponsorshipFilterImpl implements VisaSponsorshipFilter {
    ...
    private final List<String> targetCountryNames;
    ...
    public VisaSponsorshipFilterImpl(VisaDetectionChain detectionChain,
        ...
        if (visaConfig != null) {
            ...
            // Build parallel lists: patterns and country names
            List<Pattern> patterns = new ArrayList<>();
            List<String> names = new ArrayList<>();
            for (String country : visaConfig.targetCountries()) {
                String pattern = country.startsWith("\\b") ? country : "\\b" + country + "\\b";
                patterns.add(Pattern.compile(pattern, Pattern.CASE_INSENSITIVE));
                names.add(country.replaceAll("\\\\b", "").toLowerCase());
            }
            this.targetCountryPatterns = patterns;
            this.targetCountryNames = names;
            ...
            this.targetCountryNames = List.of();
    ...
    public String extractCountry(String location) {
        ...
        for (int i = 0; i < targetCountryPatterns.size(); i++) {
            if (targetCountryPatterns.get(i).matcher(lower).find()) {
                return targetCountryNames.get(i);
            }
        }
        return null;
    }
