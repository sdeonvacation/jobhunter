...
@Slf4j
@Component
public class GenericAiExtractor implements JobExtractor {
    ...
    public boolean canExtract(CareerEndpoint endpoint) {
        return endpoint.getAtsType() == AtsType.CUSTOM
    ...
    public ExtractionResult extract(CareerEndpoint endpoint) {
        ...
        if (!aiProvider.isAvailable()) {
            log.warn("AI provider not available, skipping CUSTOM endpoint [{}]", endpoint.getId());
            return ExtractionResult.error("AI provider not available", elapsed(start));
        }
        ...
        try {
            ...
            List<RawJobData> jobs = response.jobs().stream()
                    .filter(j -> j.title() != null && !j.title().isBlank())
                    .map(j -> mapToRawJobData(j, endpoint.getUrl()))
                    .toList();
    ...
    private RawJobData mapToRawJobData(AiExtractionResponse.AiJobEntry entry, String endpointUrl) {
