...
class GreenhouseExtractorTest {
    ...
    void setUp(WireMockRuntimeInfo wmInfo) {
        WebClient webClient = WebClient.builder()
                ...
                .build();
        extractor = new TestableGreenhouseExtractor(webClient, new ObjectMapper(), wmInfo.getHttpBaseUrl());
    }

    @Test
    void supportedTypes_returnsGreenhouse() {
        assertThat(extractor.supportedTypes()).containsExactly(AtsType.GREENHOUSE);
    }

    @Test
    void canExtract_validEndpoint() {
    ...
    @Test
    void canExtract_missingSlug_returnsFalse() {
        var endpoint = CareerEndpoint.builder()
                .atsType(AtsType.GREENHOUSE)
                .atsSlug(null)
                .build();
     ...
     */
    private static class TestableGreenhouseExtractor extends GreenhouseExtractor {
        private final String baseUrl;

        TestableGreenhouseExtractor(WebClient webClient, ObjectMapper objectMapper, String baseUrl) {
        ...
        public ExtractionResult extract(CareerEndpoint endpoint) {
            // Temporarily override: build a CareerEndpoint pointing at WireMock
            // The parent class uses a formatted URL, so we intercept via WebClient baseUrl
            // Since the parent formats a full URL, we need to override the behavior
            ...
            try {
                ...
                for (var jobNode : jobsNode) {
                    ...
                    jobs.add(new RawJobData(externalId, title, location, description,
                            applyUrl, jobNode.toString(), null, null, null, postedDate));
                }

                return jobs.isEmpty()
                        ? ExtractionResult.empty(elapsed(start))
                        : ExtractionResult.success(jobs, elapsed(start));
